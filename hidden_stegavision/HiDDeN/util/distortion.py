...
def random_tamper(img, ratio):
    mask_h = 8
    mask_w = 8
    mask = torch.randn(img.shape[0], mask_h * mask_w)
    topk = int((1.0 - ratio) * mask_h * mask_w)
    print(topk)
    _, topk_idx = mask.topk(topk, dim=1)
    topk_mask = torch.zeros_like(mask, dtype=torch.bool).scatter(dim=1, index=topk_idx, src=torch.ones_like(topk_idx, dtype=torch.bool)).reshape(-1, 1, mask_h, mask_w)
    tamper_mask = util.resize_qr_unit(topk_mask, factor=14).float()
    tamper_mask = transforms.Resize((args.train.img_size, args.train.img_size))(tamper_mask)
    return tamper_mask.to(img.device) * img
